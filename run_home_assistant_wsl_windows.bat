...
echo [INFO] Garantindo Home Assistant no WSL...
wsl.exe -d %DISTRO% --cd %WSL_PROJECT_DIR% /bin/bash -lc "./scripts/ensure_home_assistant.sh"
if errorlevel 1 (
    echo [ERRO] Falha ao iniciar Home Assistant no WSL.
    exit /b 1
)

echo [OK] Home Assistant ativo.
echo [OK] URL: http://127.0.0.1:8123
